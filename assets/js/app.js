...
const API = {
    ...
    const { data, error } = await supabaseClient
      ...
      .select(`
        *,
        ...
        bids(*, assembler:profiles!bids_assembler_id_fkey(id, full_name, avatar_url, hourly_rate, rating, completed_jobs, city))
      `)
      .eq('id', jobId)
      .single();
    return { data, error };
  ...
  async getBidsForJob(jobId) {
    const { data, error } = await supabaseClient
      ...
      .select(`
        *,
        assembler:profiles!bids_assembler_id_fkey(id, full_name, avatar_url, hourly_rate, rating, completed_jobs, city, skills, bio)
      `)
      .eq('job_id', jobId)
      .order('created_at', { ascending: false });
    return { data, error };
