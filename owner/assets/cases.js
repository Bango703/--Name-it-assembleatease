  ...
  function renderList() {
    ...
    list.innerHTML = state.cases.map(function(item) {
      ...
      return '<button type="button" class="cases-list-item' + (item.id === state.selectedId ? ' active' : '') + '" data-case-id="' + attr(item.id) + '">' +
        ...
        (item.attention ? '<div class="cases-list-meta"><strong>Needs attention:</strong> ' + esc(item.attention) + '</div>' : '') +
  ...
  function renderDetail(item, events) {
    ...
    detail.innerHTML =
      '<div class="cases-detail-title-row">' +
        ...
        '</div>' +
      '</div>' +
      (item.attention ? '<div class="cases-detail-section"><div class="cases-section-label">Needs attention</div><div class="cases-resolution" role="status">' + esc(item.attention) + '</div></div>' : '') +
      (item.related ? '<div class="cases-detail-section"><div class="cases-section-label">Same phone call</div>' +
        (item.related.unavailable ? '<div class="cases-resolution">Related requests could not be loaded. Refresh before assuming none were saved.</div>' :
          (item.related.cases || []).length ? item.related.cases.map(function(related) {
            return '<button type="button" class="cases-list-item" data-case-id="' + attr(related.id) + '">' + esc(related.ref) + ' - ' + esc(related.subject) + ' (' + esc(related.statusLabel) + ')</button>';
          }).join('') : '<div class="cases-resolution">No other linked call record or confirmed request is currently available. This does not prove the call outcome.</div>') + '</div>' : '') +
      '<div class="cases-detail-section">' +
