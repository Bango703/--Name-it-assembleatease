-- AssembleAtEase migration 055
-- Fixes migration 052's runtime-ambiguous financial_event_audit.booking_id
-- reference. The financial rules and mutation remain otherwise identical.
--
-- No booking or ledger row is changed when this migration is applied. It only
-- replaces the SECURITY DEFINER function used by the authenticated owner flow.
...
BEGIN
  IF to_regclass('public.bookings') IS NULL
     OR to_regclass('public.owner_manual_payment_events') IS NULL
     OR to_regclass('public.financial_event_audit') IS NULL
     OR to_regclass('public.platform_schema_state') IS NULL THEN
    RAISE EXCEPTION 'Apply launch migrations through 054 before migration 055';
  END IF;
  IF NOT EXISTS (
    SELECT 1
      FROM public.platform_schema_state schema_state
     WHERE schema_state.migration_number = 54
  ) THEN
    RAISE EXCEPTION 'Migration 054 must be recorded before migration 055';
  END IF;
END;
$$;
...
BEGIN
  ...
  SELECT booking_row.* INTO v_booking
  ...
  -- Qualifying this lookup is the P0 fix. The RETURNS TABLE output also has a
  -- booking_id variable, so an unqualified reference fails at runtime.
  SELECT audit_row.* INTO v_existing
    FROM public.financial_event_audit audit_row
   WHERE audit_row.booking_id = p_booking_id
     AND audit_row.idempotency_key = v_operation_key
     AND audit_row.event_type = 'owner_manual_balance_discounted'
   LIMIT 1;
  IF FOUND THEN
    RETURN QUERY SELECT
  ...
  IF v_booking.payment_collected IS TRUE
     ...
     OR EXISTS (
       SELECT 1
         FROM public.payout_ledger payout_row
        WHERE payout_row.booking_id = v_booking.id
     )
  ...
  SELECT
    COALESCE(SUM(payment_event.amount_cents), 0),
    COALESCE(SUM(payment_event.refunded_cents), 0),
    COALESCE(SUM(payment_event.processing_fee_cents), 0),
    COUNT(DISTINCT payment_event.payment_method),
    MIN(payment_event.payment_method),
    MAX(COALESCE(payment_event.stripe_created_at, payment_event.created_at))
    INTO v_gross, v_refunded, v_fee_total,
         v_method_count, v_single_method, v_latest_collection
    FROM public.owner_manual_payment_events payment_event
   WHERE payment_event.booking_id = v_booking.id;
  ...
  UPDATE public.bookings booking_row
     SET standard_price_cents = COALESCE(booking_row.standard_price_cents, v_booking.total_price),
         ...
         refund_amount = 0
   WHERE booking_row.id = v_booking.id;
...
VALUES (55, 'owner_manual_discount_rpc_ambiguity_fix')
