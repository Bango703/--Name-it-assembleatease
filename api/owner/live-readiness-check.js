...
const REQUIRED_SCHEMA_MIGRATION = 44;
  ...
  try {
    ...
    if (error || !data) {
      return fail(
        ...
        'Apply migrations 038-044 in order and rerun readiness.',
      );
    }
    return pass('DATABASE_SCHEMA_044', `Launch schema ${data.migration_number} is applied.`);
  } catch (error) {
    return fail(
      'DATABASE_SCHEMA_044',
      'Could not verify the launch database schema.',
      'Confirm Supabase connectivity and apply migrations 038-044 in order.',
    );
  }
}
